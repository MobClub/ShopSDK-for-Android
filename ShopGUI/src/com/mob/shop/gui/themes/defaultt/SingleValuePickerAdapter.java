...
import com.mob.shop.gui.themes.defaultt.components.FlywheelView;
import com.mob.shop.gui.themes.defaultt.components.FlywheelView.OnSelectedListener;
import com.mob.tools.utils.ResHelper;

import java.util.ArrayList;

public class SingleValuePickerAdapter extends ScrollUpAndDownDialogAdapter<SingleValuePicker> implements OnSelectedListener, OnClickListener {
	...
	protected void initBodyView(LinearLayout llBody) {
		...
		tvCancel.setText(ResHelper.getStringRes(context, "shopsdk_default_cancel"));
		...
		tvConfirm.setText(ResHelper.getStringRes(context, "shopsdk_default_complete"));
			...
			if (view.equals(wheels[i])) {
				if (selections[i] != selection) {
					...
					if (i < wheels.length - 1) {
						@SuppressWarnings("unchecked") ArrayList<Choice> choices = (ArrayList<Choice>) wheels[i]
								.getTag();
