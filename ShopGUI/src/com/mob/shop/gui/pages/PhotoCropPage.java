...
public class PhotoCropPage extends Page<PhotoCropPage> {
	...
	private void setListenerAndShow(final boolean cameraType, final OnImageGotListener listener) {
		...
		try {
			permissionGranted = DeviceHelper.getInstance(MobSDK.getContext()).checkPermission("android.permission" +
...
//			t.printStackTrace();
	...
	private void afterAlbum(Intent data) {
		...
		int resId = ResHelper.getStringRes(getContext(), "shopsdk_default_operation_faield");
		builder.setTitle(getContext().getString(resId));
		resId = ResHelper.getStringRes(getContext(), "shopsdk_default_image_not_exists");
	...
	private void uploadImage() {
		ShopSDK.uploadImage(getOutput(), new OperationCallback<String>() {
			@Override
			public void onSuccess(String data) {
				super.onSuccess(data);
				listener.onImageUploadSuccess(null, data);
			}
			...
			public void onFailed(Throwable t) {
				...
				toastMessage(ResHelper.getStringRes(activity, "shopsdk_default_save_img_failed"));
			}
		});
	}
}
